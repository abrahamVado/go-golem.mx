...
func (h *Handler) Register(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, ErrCompanyNameRequired),
			...
			errors.Is(err, ErrRegisterEmailExists),
...
func (h *Handler) Me(c *gin.Context) {
	me, err := h.svc.Me(tenancy.CompanyID(c), tenancy.UserID(c))
	if err != nil {
		response.Internal(c, "Failed to load profile")
		return
	}
...
func (h *Handler) UpdateMe(c *gin.Context) {
	...
	me, err := h.svc.UpdateMe(tenancy.CompanyID(c), tenancy.UserID(c), req)
	if err != nil {
		switch {
...
func (h *Handler) ChangeMyPassword(c *gin.Context) {
	...
	if err := h.svc.ChangeMyPassword(tenancy.CompanyID(c), tenancy.UserID(c), req); err != nil {
		switch {
...
func (h *Handler) UploadMyAvatar(c *gin.Context) {
	...
	me, err := h.svc.UpdateMyAvatar(tenancy.CompanyID(c), tenancy.UserID(c), file)
	if err != nil {
		switch {
		case errors.Is(err, ErrInvalidAvatar):
			response.BadRequest(c, err.Error())
		default:
			response.Internal(c, "Failed to update avatar")
		}
		return
	}
